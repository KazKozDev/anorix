...
class VectorStore:
    ...
    def delete_by_filter(self, filter_metadata: Dict[str, Any]) -> bool:
        ...
        For Chroma, uses where-filter delete. For FAISS, returns False (not supported).
        """
        try:
            if not filter_metadata:
                self.logger.warning("delete_by_filter called without filter; aborting")
                return False
            if self.store_type == "chroma":
                try:
                    # Access underlying collection to delete by where-filter
                    self.vector_store._collection.delete(where=filter_metadata)
                    return True
                except Exception as e:
                    self.logger.error(f"Chroma delete_by_filter failed: {e}")
                    return False
            elif self.store_type == "faiss":
                self.logger.warning("FAISS does not support delete_by_filter; consider clearing collection or switching to Chroma")
                return False
            else:
                return False
        except Exception as e:
