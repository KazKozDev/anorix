...
class RAGRetrievalTool:
    ...
    class RAGRetrievalInput(BaseModel):
        ...
        filter_metadata: Optional[Union[Dict[str, str], str]] = Field(default=None, description="Metadata filters as dictionary or JSON string")

        @field_validator('filter_metadata')
        @classmethod
        def validate_filter_metadata(cls, v):
            ...
            if isinstance(v, str):
                try:
                    ...
                    # If it's not valid JSON, try to parse as simple key=value format
                    result = {}
                    pairs = v.split(',')
                    for pair in pairs:
                        if '=' in pair:
...
class RAGManagementTool:
    ...
    class RAGManagementInput(BaseModel):
        ...
        action: str = Field(description="Action to perform: add_file, add_files, add_directory, add_text, info, clear, delete_by_filter, delete_user_facts")
        ...
        metadata: Optional[Dict[str, Any]] = Field(default=None, description="Optional metadata for text documents (used with add_text)")
    ...
    def _manage_rag_structured(
        ...
        try:
            ...
            if action == 'info':
                ...
                if title:
                    meta['title'] = title
                return self.rag_tool.add_text_document(content, meta)
            
            elif action == 'delete_by_filter':
                if not metadata:
                    return "Error: metadata parameter required for delete_by_filter action"
                ok = self.rag_tool.vector_store.delete_by_filter(metadata)
                return "Deleted documents by filter" if ok else "Failed to delete by filter"
            
            elif action == 'delete_user_facts':
                ok = self.rag_tool.vector_store.delete_by_filter({"type": "user_fact"})
                return "Deleted all user facts" if ok else "Failed to delete user facts (backend may not support filtered deletion)"
            
            else:
                return (
                    f"Unknown action: {action}. Available actions: "
                    "add_file, add_files, add_directory, add_text, info, clear, delete_by_filter, delete_user_facts"
                )
