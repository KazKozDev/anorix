...
from .tools.emotional_intelligence import get_emotional_intelligence_tool
from .tools.personality_system import get_personality_system_tool
from .tools.proactive_care import get_proactive_care_tool
from .tools.fact_extractor import LLMFactExtractor
...
class ToolManager:
    ...
    def _load_default_tools(self):
        ...
        try:
            ...
            # Add virtual friend tools (without personal_memory)
            try:
                friend_tools = [
                ...
                default_tools.extend(friend_tools)
                self.logger.info(f"Added {len(friend_tools)} virtual friend tools")
            except Exception as e:
                self.logger.warning(f"Failed to load virtual friend tools: {e}")

            # Add LLM-based fact extractor tool
            try:
                fact_extractor_tool = LLMFactExtractor().get_tool()
                default_tools.append(fact_extractor_tool)
                self.logger.info("Added LLM fact extractor tool")
            except Exception as e:
                self.logger.warning(f"Failed to initialize fact extractor tool: {e}")
