...
MEMORY COMMANDS (via RAG):
  /mem add <text>         - Store a new user fact into the RAG KB
  /mem search <query>     - Search user facts in RAG (filtered by type=user_fact)
        ...
        while True:
            try:
                ...
                if user_input.lower() in ['/quit', '/exit', '/q']:
                    ...
                    if len(parts) == 1:
                        print("Use /mem add <text> | /mem search <q>")
                        continue
                    ...
                    try:
                        if hasattr(self.agent, 'tool_manager'):
                            rr_tool = self.agent.tool_manager.get_tool('rag_retrieval')
                            rm_tool = self.agent.tool_manager.get_tool('rag_management')
                    except Exception:
                        pass
                    try:
                        if action == 'add' and len(parts) >= 3:
                            text = parts[2]
                            if not rm_tool:
                                print("rag_management tool is not available.")
                            else:
                                out = rm_tool.func(action='add_text', content=text, metadata={"type":"user_fact","source":"cli"})
                                print(out)
                        elif action == 'search' and len(parts) >= 3:
                            query = parts[2]
                            if not rr_tool:
                                print("rag_retrieval tool is not available.")
                            else:
                                out = rr_tool.func(query=query, k=5, with_scores=False, filter_metadata={"type":"user_fact"})
                                print(out)
                        else:
                            print("Unknown /mem command. Use: /mem add <text> | /mem search <q>")
                    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        '--model', '-m', 
        default=None,
        help="Ollama model name (default: from env or config)"
    )
    ...
    # Resolve model from args or centralized settings
    model = args.model or load_llm_settings().get('model_name')
    interactive_agent = InteractiveAgent(model_name=model, verbose=verbose)
